...
void Model::AssetManager::load_texture(TextureID id, const std::string& path) {
    textures.insert({id, make_shared<SDL2pp::Texture>(*renderer, path)});
}

void Model::AssetManager::load_texture(TextureID id, Shared<SDL2pp::Texture> texture) {
    textures.insert({id, texture});
}

void Model::AssetManager::load_font(FontID id, const std::string& path, int size) {
    fonts.insert({id, make_shared<SDL2pp::Font>(path, size)});
}

void Model::AssetManager::load_font(FontID id, Shared<SDL2pp::Font> font) {
    fonts.insert({id, font});
}

Shared<SDL2pp::Texture> Model::AssetManager::get_texture(TextureID id) {
    return textures.at(id);
}

Shared<SDL2pp::Font> Model::AssetManager::get_font(FontID id) {
    return fonts.at(id);
}

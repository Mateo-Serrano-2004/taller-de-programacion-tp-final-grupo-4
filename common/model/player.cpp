...
Model::Player::Player(bool shooting, short_id_t id, Model::RoleID role_id, angle_t angle,
                      uint16_t money, const std::string& name, const Physics::Vector2D& position,
                      Shared<Model::Weapon> weapon, uint8_t health, Model::TeamID team):
        alive(true),
        shooting(shooting),
        id(id),
        role_id(role_id),
...
DTO::PlayerDTO Model::Player::to_dto() const {
    return DTO::PlayerDTO(id, static_cast<short_id_t>(role_id), angle, money, position.get_x(),
                          position.get_y(), name, current_weapon->to_dto(), shooting, health,

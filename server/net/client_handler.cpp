...
void ClientHandler::handle_create_game(const Model::CreateGameEvent& event) {
...
void ClientHandler::handle_join_game(const Model::JoinGameEvent& event) {
    sender = new ClientHandlerSender(protocol);
...
void ClientHandler::handle_username(const Model::UsernameEvent& event) {
...
void ClientHandler::handle_game_event(const GameEventVariant& event) {
    if (std::holds_alternative<Model::QuitEvent>(event)) {
...
void ClientHandler::handle_event(const EventVariant& event) {
    ...
    //            event);
    std::visit(overloaded{[this](const Model::UsernameEvent& e) { handle_username(e); },
                          [this](const Model::CreateGameEvent& e) { handle_create_game(e); },
                          [this](const Model::JoinGameEvent& e) { handle_join_game(e); },
                          [this](const GameEventVariant& e) { handle_game_event(e); }},
               event);
}

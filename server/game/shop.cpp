...
void Shop::process_weapon_purchase(FullPlayer& player, Model::WeaponID weapon_id) const {
    uint16_t price = get_price(weapon_id);
    if (player.can_pay(price)) {
        player.substract_money(price);
        player.equip_new_weapon_and_drop_previous(WeaponFactory::create(weapon_id));
    }
}
